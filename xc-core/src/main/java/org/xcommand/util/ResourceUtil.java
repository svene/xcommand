...
public final class ResourceUtil {
    ...
    public static InputStream newInputStreamFromResourceLocation(String aResourceLocation) {
        ...
        if (is == null) {
            throw new RuntimeException(
                    "could not load resource '%s' via context classloader".formatted(aResourceLocation));
        }
        return is;
    }
    ...
    public static InputStream newInputStreamFromFilename(String aFileName) throws FileNotFoundException {
        ...
        if (!file.exists()) {
            throw new RuntimeException(
                    "file not found: '%s' (workingdir: '%s')".formatted(aFileName, new File(".").getAbsolutePath()));
        }
